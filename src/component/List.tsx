...
const List = (props: LeaderboardItem) => {
    ...
    const username= useSelector((state)=>state.searchUser.username)
        ...
        renderItem={({ item,index }: { item: LeaderboardItem,index:number })=>{
            return (
                <View style={{flex:1,justifyContent:'space-between',flexDirection:'row'}}>
                    {
                      (searchedUserIndex==null && username=='') || (searchedUserIndex &&  username)  ?
                      <>
                      ...
                      :
                      <Text style={{color:'black',flex:0.4,}}><Text style={{fontWeight:'bold'}}>{'\n'}Name{'\n'}</Text>{item?.name}</Text>
                    }
